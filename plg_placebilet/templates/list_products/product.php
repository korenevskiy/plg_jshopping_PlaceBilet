...
<div class="blockcard"> 
    ...
    ?>
        ...
        <div class="name">
            ...
            <?php if ($this->config->product_list_show_product_code){?><span class="jshop_code_prod">(<?php print __('JSHOP_EAN')?>: <span><?php print $product->product_ean;?></span>)</span><?php }?>
        </div>
...
<div 
  class="product productitem_<?php print $product->product_id?>">
	...
	<div class="old_price"><?php if ($this->config->product_list_show_price_description) print __('JSHOP_OLD_PRICE').": ";?><span><?php print formatprice($product->product_old_price)?></span></div>
        <?php }?>
        ...
        <?php if ($product->product_price_default > 0 && $this->config->product_list_show_price_default){?>
			...
			<div class="default_price"><?php print __('JSHOP_DEFAULT_PRICE').": ";?><span class="cost"><?php print formatprice($product->product_price_default)?><span></span></div>
        <?php }?>
        <?php if ($product->_display_price){?>
            <div class = "jshop_price"> 

                <?php if ($this->config->product_list_show_price_description) print __('JSHOP_PRICE').": ";?>
                <?php if ($product->show_price_from) print __('JSHOP_FROM')." ";?>
...
//toPrint($de->format(' j: F  H:i        ', true),'Ticket '.$product->product_id,true, 'message',true);
        ...
        <?php if ($product->manufacturer->name){?>
            <div class="manufacturer_name"><?php print __('JSHOP_MANUFACTURER');?>: <span><?php print $product->manufacturer->name?></span></div>
        <?php }?>
        <?php if ($product->product_quantity <=0 && !$this->config->hide_text_product_not_available){?>
            <div class="not_available"><?php print __('JSHOP_PRODUCT_NOT_AVAILABLE');?></div>
        <?php }?>
        
        <?php if ($this->config->show_plus_shipping_in_product){?>
            <span class="plusshippinginfo"><?php print sprintf(__('JSHOP_PLUS_SHIPPING'), $this->shippinginfo);?></span>
        <?php }?>
        <?php if ($product->basic_price_info['price_show']){?>
            <div class="base_price"><?php print __('JSHOP_BASIC_PRICE')?>: <?php if ($product->show_price_from) print __('JSHOP_FROM');?> <span><?php print formatprice($product->basic_price_info['basic_price'])?> / <?php print $product->basic_price_info['name'];?></span></div>
        <?php }?>
        <?php if ($this->config->product_list_show_weight && $product->product_weight > 0){?>
            <div class="productweight"><?php print __('JSHOP_WEIGHT')?>: <span><?php print formatweight($product->product_weight)?></span></div>
        <?php }?>
        <?php if ($product->delivery_time != ''){?>
            <div class="deliverytime"><?php print __('JSHOP_DELIVERY_TIME')?>: <span><?php print $product->delivery_time?></span></div>
        <?php }?>
        ...
        <div class="count_commentar">
            <?php print sprintf(__('JSHOP_X_COMENTAR'), $product->reviews_count);?>
        </div>
        <?php }?>
		...
		<div class="vendorinfo"><?php print __('JSHOP_VENDOR')?>: <a href="<?php print $product->vendor->products?>"><?php print $product->vendor->shop_name?></a></div>
        <?php }?>

        <?php if ($this->config->product_list_show_qty_stock){?>
		<div class="qty_in_stock"><?php print __('JSHOP_QTY_IN_STOCK')?>: <span><?php print sprintQtyInStock($product->qty_in_stock)?></span></div>
        <?php }?>
...
<div class="buttons">
	...
	<a class="button_buy" href="<?= $product->buy_link?>"><?php print __('JSHOP_BUY')?></a> &nbsp;
    <?php }?>
