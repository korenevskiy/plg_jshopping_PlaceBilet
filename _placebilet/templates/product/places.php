...
$acc_open = $acc_class = '';
        ...
        foreach($places_sort as $attr_id=> $attr_arr){
            ...
            $attr = $attributes_sort[$attr_id];
			
			
			
            $ttls = "<span class='name'>$attr->name</span> <span class='descr'>$attr->description</span>";
            //$ttls =  "<span class='name'>$attr->name <span class='descr'>$attr->description</span></span>";
...
//                $groups_sort= array();
			...
			echo "<input name=jshop_attr_id[$attr_id][] value=0 type=hidden>";
   
            foreach($attr_arr as $group_id => $group_attr){
				
				$group_name = $groups_sort[$group_id];
				
               // var_dump($group_attr);
                echo "<div class='attributes_side'>  ";
					echo $group_name ? "<span class='side'>$group_name</span>" : '';
...
//                    toPrint($config->currency_format,'currency_format',0);
                    ...
                    if($price_show) echo "<span class='cost rub d-table-cell'>".JSHelper::formatprice($add_price,NULL,0, 1)." - </span>";
					
					echo '<div class="d-table-cell">';

                    $places = [];
                    foreach ($places_attr as $place){
						$place_name = $place->type == 'int' ? intval(trim($place->place_name)) :  strval(trim($place->place_name));
						$places[$place_name] = $place;
                    }
                    
//\Joomla\CMS\Factory::$application->enqueueMessage("<pre>tmpPlaces 261 <b>\$places_attr</b> ".print_r($places_attr,true)."</pre>");
//\Joomla\CMS\Factory::$application->enqueueMessage("<pre>tmpPlaces 261 <b>\$places</b> ".print_r($places,true)."</pre>");
...
//						 name='jshop_place_id[$place->attr_value_id]'
						echo "<input name='jshop_attr_id[$place->attr_id][$place->attr_value_id]'		data-name='jshop_place_id[$place->id]'	 id='jshop_place_id_$place->id' class='PlaceNumber' value='0' type='number' min='0'  aria-label='".JText::_('JSHOP_PLACE_BILET_COUNT')."' "
						. "  data-price='$place->addprice' data-cur='$cur_name' data-av_id='$place->attr_value_id' data-pa_id='$place->product_attr2_id' data-tip='$place->attr_name $group_name $price ' data-pcs='".strip_tags(JText::_('JSHOP_PLACE_PCS'))."'>";
...
//						 name='jshop_place_id[$place->attr_value_id]'
						. "<input type='checkbox' name='jshop_attr_id[$place->attr_id][$place->attr_value_id]'  data-name='jshop_place_id[$place->id]' id='jshop_attr_id_$place->id' value='1' names='jshop_attr_id[]' class='checkboxplace  '  "
//							. "onclick=\"if (this.checked) setPlaceValue($place->attr_value_id, $place->id, $add_price, true); else setPlaceValue($place->attr_value_id, $place->id, $add_price, false); \" "
							. "data-tip='$place->attr_name $group_name $price '  data-price='$place->addprice' data-cur='$cur_name' data-av_id='$place->attr_value_id' data-pa_id='$place->product_attr2_id' "
							. " checkeds>"
//							. "$place->place_name"
...
//							. "<span class='ext_row hide'>$place->attr_name $group_name</span>"
//							. "<span class='ext__ryad hide'>$place->attr_name $group_name - $add_price $carency_marka </span>"
							. ""; 
						echo "</label>";
