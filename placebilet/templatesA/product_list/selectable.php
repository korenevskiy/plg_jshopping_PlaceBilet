...
<div class="js-stools clearfix jshop_block_filter">
    ...
    <div class="js-stools-container-bar">
        <div class="filter-search btn-group pull-left">
            <input type="text" id="text_search" name="text_search" placeholder="<?php print JText::_('JSHOP_SEARCH')?>" value="<?php echo htmlspecialchars($text_search);?>" />
        </div>

        <div class="btn-group pull-left hidden-phone">
            <button class="btn hasTooltip" type="submit" title="<?php print JText::_('JSHOP_SEARCH')?>">
                <i class="icon-search"></i>
            </button>
            <button class="btn hasTooltip" onclick="document.id('text_search').value='';this.form.submit();" type="button" title="<?php print JText::_('JSHOP_CLEAR')?>">
...
<?php foreach ($rows as $row){?>
  <tr class="row<?php echo $i % 2;?>">
   ...
   <?php }?>
   <td>		
    <?php echo JSHelper::formatprice($row->product_price, sprintCurrency($row->currency_id));?>
   </td>
   <td>
