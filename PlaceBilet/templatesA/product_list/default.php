...
//toPrint($this->filter_order,'$this->filter_order',0,true);
...
$manufacturer_id=1;//$this->manufacturer_id; // Изменено
$count=count ($rows);
$i=0;
$saveOrder=$this->filter_order_Dir=="asc" && $this->filter_order=="ordering";
JHtml::_('formbehavior.chosen', '.chosen-select');
?>
<!--<div style="display: flex; justify-content: space-between; align-content: stretch; align-items: stretch;">-->

    <div id="j-sidebar-container" class="span2"  >
<!--        <div style="position: static;  position: sticky;">-->
            <?php echo $this->sidebar; ?>
<!--        </div>-->
    </div>
    <div id="j-main-container" class="span10"  style=" ">
<form action="index.php?option=com_jshopping&controller=products" method="post" name="adminForm" id="adminForm">
<?php if(isset($row->tmp_html_start))  print $this->tmp_html_start?>
...
<div class="js-stools clearfix jshop_block_filter">
    ...
    <div class="js-stools-container-filters">
        ...
        <?php  if(isset($row->tmp_html_filter)) print $this->tmp_html_filter?>
...
<thead> 
  <tr>
    ...
    <?php  if(isset($row->tmp_html_col_after_title)) print $this->tmp_html_col_after_title?>
    <?php if (!$category_id){?>
    <th width="80">
    ...
    <th width="60">
        <?php echo JHTML::_( 'grid.sort', _JSHOP_DATE, 'date', $this->filter_order_Dir, $this->filter_order);
         //   echo JHTML::_( 'grid.sort', _JSHOP_DATE, 'date', $this->filter_order_Dir, $this->filter_order);?>
    </th>
    <?php if ($category_id) {?>
...
<?php foreach($rows as $row){?>
  <tr class="row<?php echo $i % 2;?>">
   ...
   <td>
     ...
     <div><?= $row->short_description;?></div>
     <?php if(isset($row->RepertoireId)) echo "<div style='opacity: 0.5'>RepertoireId: $row->RepertoireId</div>" ;?>
     <?php if(isset($row->RepertoireId) && $row->product_date_added) echo "<div style='opacity: 0.5'>Add: $row->product_date_added </div>";?>
     <?php if(isset($row->RepertoireId) && $row->date_modify) echo "<div style='opacity: 0.5'>Mod: $row->date_modify </div>"; ?> 
   </td>
   <?php if(isset($row->tmp_html_col_after_title)) print $row->tmp_html_col_after_title?>
   <?php if (!$category_id){?>
   <td>
   ...
   <td>  
    ...
    $week_Days = date('N', strtotime($row->date_event)); 
    $opacity = 0.8;
    if($week_Days == 6)
        $opacity = 0.3;
    if($week_Days < 6)
    ...
    echo "<div style='width:100px;background-color:$color;'>".strftime("%e ",strtotime($row->date_event)). JText::_(strftime("%B",strtotime($row->date_event))). strftime(" %G",strtotime($row->date_event)). '</div>'
            . '<b style="font-size:24px;">'.strftime("%R",strtotime($row->date_event))."</b><span style=' ' class=''> ".JText::_(strftime("%a",strtotime($row->date_event)))."</span>";//formatdate($row->date_event, 1). strftime("%R:%M",$row->date_event) Изменено product_date_added на date_event изменено-добавленно
    ?>
   </td>
   <?php if ($category_id) {?>
...
<tfoot>
<tr>
    <?php if(isset($row->tmp_html_col_before_td_foot))  print $this->tmp_html_col_before_td_foot?>
    <td colspan="18">
		<div class = "jshop_list_footer"><?php echo $pageNav->getListFooter(); ?></div>
        <div class = "jshop_limit_box"><?php echo $pageNav->getLimitBox(); ?></div>
	</td>
    <?php  if(isset($row->tmp_html_col_after_td_foot))  print $this->tmp_html_col_after_td_foot?>
</tr>
</tfoot>
</table>
