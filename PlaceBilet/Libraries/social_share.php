...
//        $cat_id = FALSE;
//        JUri::base();
        $link = ($cat_id)?(JUri::base().SEFLink("index.php?option=com_jshopping&controller=category&task=view&category_id=$cat_id")):JURI::current();
        $link = ($cat_id)?(JUri::base().SEFLink("index.php?option=com_jshopping&controller=product&task=view&category_id=$cat_id&product_id=$product_id", 1)):JURI::current();
//        toPrint($link,'$link');
        $url = &$link;
...
//		$url = '//'.$uri->getHost().$uri->getPath();
    ...
    private function getVkShareButton($params, $url){
