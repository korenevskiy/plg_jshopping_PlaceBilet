...
require_once (JPATH_PLUGINS.'/jshopping/PlaceBilet/Libraries/SoapClient.php');
...
//    public static $UpdateProducts=FALSE;
    ...
    public static function Instance(JRegistry $paramsPlugin, $ErrorView = FALSE){//JRegistry JObject
        static::$debug_off  = isset(PlaceBiletHelper::JRequest()->getInt('deb', 0)) || FALSE;
...
//toPrint('yes 1:');
        ...
        if(!static::$Zriteli_repertoire_download_enabled && !$Zriteli_repertoire_download_enabled)
            return [-1,-1,__LINE__];
...
//        toPrint(array_keys($products_NOexist),'$products_NOexist  [product_id]'); // Count^388  
        ...
        if ($products_onlyDB) {
            ...
            if (static::$bilet_delete == 'prod' && $ids) { // Удаление прошедших кроме одного каждого репертуара, один каждого репертуара скрывается
                $query = "
                    UPDATE #__jshopping_products p SET p.product_publish = FALSE
                    WHERE product_id IN (" . join(',', $ids) . " ) ; ";
                JFactory::getDBO()->setQuery($query)->execute();
                ...
                $query = "
                    ...
                    WHERE p.product_publish = FALSE AND p.RepertoireId !=0 AND 
                        p.product_id NOT IN (SELECT MIN(g.product_id) FROM #__jshopping_products g GROUP BY g.RepertoireId); ";
                $query = "
                    DELETE #__jshopping_products FROM #__jshopping_products
                    JOIN(
                        SELECT product_id FROM #__jshopping_products 
                        WHERE product_publish = FALSE AND RepertoireId !=0 AND product_id NOT IN (
                            SELECT MIN(n.product_id) product_id FROM #__jshopping_products n GROUP BY n.RepertoireId)
                        ) s USING (product_id)
                    WHERE #__jshopping_products.product_id = s.product_id; ";        
                JFactory::getDBO()->setQuery($query)->execute();
            }
        }

        if (static::$bilet_old == 'del') { // Удаление прошедших
            $query = "
                    DELETE FROM `#__jshopping_products` 
                    WHERE p.date_event < DATE( NOW()) /*AND p.RepertoireId !=0 */ ; ";
            JFactory::getDBO()->setQuery($query)->execute();
        }
        
        if (static::$bilet_old == 'hide') { // Скрытие прошедших
            $query = "
                    UPDATE #__jshopping_products p SET p.product_publish = FALSE
                    WHERE p.date_event < DATE( NOW()) /*AND p.RepertoireId !=0 */ ; ";
            JFactory::getDBO()->setQuery($query)->execute();
        ...
        if (static::$bilet_old == 'prod') {// Удаление прошедших кроме одного каждого репертуара, один каждого репертуара скрывается
            $query = "
                    UPDATE #__jshopping_products p SET p.product_publish = FALSE
                    WHERE p.date_event < DATE( NOW()) /*AND p.RepertoireId !=0 */ ; ";
            JFactory::getDBO()->setQuery($query)->execute();
...
//            toPrint(count($products_NOexist),'New Repertoires --------->>>>>: Stop LoadAllProducts');  
            ...
            $Zriteli_repertoire_download_enabled = FALSE;
        }
...
//            $offer->StageCatId = $StageCatId;
    ...
    static function RemoveOldProducts(){
        $params = static::$Params;
        $time_type = $params->get('bilet_old_hidetime','time')[0]=='t'?'time':'letter';// Тип интервала удаления 
        $time_hours = (int)substr($params->get('bilet_old_hidetime',3), 1);        // Колличество часов
        
        $bilet_old      = $params->get('bilet_old'); //[ "cat","prod","hide","del",""]; // 
        $bilet_delete   = $params->get('bilet_delete');//["cat","prod","hide","del",""]; // 
...
//                $prod->params['OffersId']         = array_keys($prod->Offers);
    ...
    public static function UpdateLoadStages(JRegistry $params = NULL, $gley = " — "){
        ...
        $catId_stages = (int)$params->get('CategoriesStages', 0);
        $StagePlaces_id = $params->get('StagePlaces', []);
        $view_Name_Place = $params->get('view_Name_Place', 0);
        $gley = ($view_Name_Place)? $gley : '';
//        toPrint($catId_stages, ' $catId_stages    '.count($catId_stages),"<br><br>");
//        toPrint($StagePlaces_id, ' $StagePlaces_id    '.count($StagePlaces_id),"<br><br>");
        if($catId_stages == -1)
            return;
        ...
        try {//print
            if ($view_Name_Place)
                $NamePlaces = SoapClientZriteli::Call_GetPlaceList(FALSE); // Площадки(театры)
...
//        toPrint($newCats,'$newCats');
        ...
        foreach($newCats as $stageId => $stage){
            if(!isset($stage)) toPrint ($stage,'$stage');
            if(!isset($cats[$stageId])) toPrint ($stage,'$stage');
            if(!isset($StagePlaces_id[$stageId])) toPrint ($stage,'$stage');
...
//         toPrint($offers, ' $offers    '.count($offers),"<br><br>");
...
// <editor-fold defaultstate="collapsed" desc="Запрос БД добавления в базу несуществующих атрибутов">
        ...
        foreach ($attrs_new as $sort => $attr)
            $querys[] = " ( $attr->StageId,$attr->Row,$attr->SectorId,$attr->StageCatId," . $db->quote($attr->Name) . ", " . $db->quote($attr->cats) .
                ",   4,0,0,1,0,$attr->OrderId ) "; //$db->quote($db->escape($repertoire->Name),false)
