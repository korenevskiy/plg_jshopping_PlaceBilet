...
defined('deb_123') or require_once (JPATH_SITE.'/components/com_jshopping/lib/functions.php');
...
//    public static $params = NULL;
    ...
    /**
     * Странситерация русской строки в латинску с добавлением префикса и даты в начало строки
...
//return; 
        $dispatcher->trigger('onBeforeProductAttributPlaceStore', array(&$product_id, &$attrib_place_name, &$attrib_place_value_id, &$attrib_place_id, 
...
//        toPrint($arr_query,'$arr_query',0,TRUE);
        $times = [];
        $times[] = JFactory::getDate()->format(DATE_RFC3339_EXTENDED);// 'Y-m-d\TH:i:s.vP %a %d %b %Y - %H:%M');
        ...
        foreach ($arr_query as $q){
            ...
            $db->setQuery($q);
            $db->query(); 
            $times[] = JFactory::getDate()->format(JDate::RFC3339_EXTENDED);// 'Y-m-d\TH:i:s.vP %a %d %b %Y - %H:%M');
...
//            toPrint($seat->AgentPrice . " - $cost -$CostRound) " . $seat->CostPrice);
    ...
    public static function getBonusProduct($product_date_event){ 
        ...
        $bonusEnabled = static::$param->get('bonusEnabled');
...
//            $viewmail = $this->getView('mail', $params->mailNotification)
//            $viewmail = new JViewLegacy(['name'=>'mail','layout'=>$params->mailNotification]);
