...
//$this->setRedirect("index.php?option=com_jshopping&controller=products&task=edit&product_id=".$product->product_id, _JSHOP_PRODUCT_SAVED);	
    ...
    if (!isset($post['attrib_ind_id'])) $post['attrib_ind_id'] = array(); 				// NULL заменен на array();
    if (!isset($post['attrib_ind_price'])) $post['attrib_ind_price'] = array();			// NULL заменен на array();
    if (!isset($post['attrib_ind_price_mod'])) $post['attrib_ind_price_mod'] = array();	// NULL заменен на array();
    if (!isset($post['attrib_ind_price_mod'])) $post['attrib_ind_price_mod'] = array();	// NULL заменен на array();
    
    //Изменено: Добавлены ниже 13 строк
    $post['date_event'] = $post['date_event'] ?? getJsDate();                  // Дата показа спектакля, концерта и кино
    
    $post['attrib_place_value_id']= $post['attrib_place_value_id']??[];   // Места интревалов ввиде строки (без пробелов и знаков припенания) 15o25 14991
    $post['attrib_place_price']   = $post['attrib_place_price']??[];      // Цена       9631 350
    $post['attrib_place_price_mod']= $post['attrib_place_price_mod']??[]; // Знак модификации цены    +
    $post['attrib_place_id']      = $post['attrib_place_id']??null;       // ID Атрибута                    9631
    $post['attrib_place_name']   = $post['attrib_place_name']??[];      // Места интревалов ввиде строки   //attrib_place_Int_tuple   350    15-25
    $post['attrib_place_type']   = $post['attrib_place_type']??[];      // тип, числовой или строковое место
...
//----------------------------------------------        
//$this->setRedirect("index.php?option=com_jshopping&controller=products&task=edit&product_id=".$product->product_id, _JSHOP_PRODUCT_SAVED);	        
//$this->setRedirect("index.php?option=com_jshopping&controller=products&task=edit&product_id=13043", _JSHOP_PRODUCT_SAVED);$this->redirect();return;//----------------------------------------------       

//toPrint(111111111111111111,'',0,'message',TRUE);
...
//return;
            $this->setRedirect("index.php?option=com_jshopping&controller=products&task=edit&product_id=".$product->product_id, _JSHOP_PRODUCT_SAVED.JText::sprintf('JSHOP_PLACE_ADDED',$count_row));
        }else{
            $this->setRedirect("index.php?option=com_jshopping&controller=products", _JSHOP_PRODUCT_SAVED.JText::sprintf('JSHOP_PLACE_ADDED',$count_row));
        }
    }
